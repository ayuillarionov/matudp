classdef SignalSpec < handle
    ...
    properties
        ...
        ndimsManual = [];         % if set, ndims will be forced up to this number,
                                  % useful when concatLastDim is true and we want to distinguish between multiple
        ...
        resetImmediately = false; % should this be cleared immediately after 1 tick when de-serializing
    ...
    properties(Dependent)
        ...
        isVariable
        isChar
        nDims                    % dimentionality
        dims                     % number of elements per each dimention
        bytesPerElement
    ...
    methods(Access=protected)
        % don't call constructor directly, use one of the Static methods below to construct
        function sv = SignalSpec(varargin)
            ...
            p.addParameter('ndimsManual', [], @(x) isempty(x) || (isnumeric(x) && isscalar(x)));
            ...
            ndimsManual = p.Results.ndimsManual;
            ...
            assert(isempty(sv.value) || numel(sv.isVariableByDim) == nDims, ...
                'isVariableByDim must have numel == nDims');
            assert(numel(sv.maxSize) == nDims, ...
                'maxSize must have numel == nDims');
        end
    end
    ...
    methods(Static) % Param builder methods
        ...
        function sv = ParamEnum(enumName, varargin)
            ...
            % use either empty or default value if not specified, empty is when the length is variable.
            % NOTE: Can't use .empty in codegen as of R2015a. Use default value.
            v = p.Results.value;
            if isempty(v)
...
%                end
        ...
        function sv = AnalogVectorMultiChannel(value, varargin)
            % manually ensure concatenation is over columns
            sv = BusSerialize.SignalSpec.Analog(value, varargin{:}, "ndimsManual", 2);
        end
