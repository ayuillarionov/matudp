...
#include <pthread.h> // POSIX treads
...
int PTHREAD_MUTEX_LOCK(pthread_mutex_t *mutex) {
	//logInfo("Signal: Attempting to lock mutex\n");
	...
	while(val != 0) {
		...
		if (val == EINVAL) {
			while (true)
				logError("Signal Error: INVALID MUTEX!!!\n");
		}

		double t = tocCheck();
		if (t > 1) {
			logError("Signal Error: Hanging waiting for mutex!\n");
			//print_trace();
			tic();
		}
	}
	...
	//logInfo("Signal: Succeeded in locking mutex\n");
	return val;
}

int PTHREAD_MUTEX_UNLOCK(pthread_mutex_t *mutex) {
	//logInfo("Signal: Attempting to unlock mutex\n");
	int val = pthread_mutex_unlock(mutex);
	//logInfo("Signal: Succeeded in unlocking mutex\n");
	return val;
}
...
// these should match the defined constants DTID_* in signal.h
...
bool getGroupTypeName(uint8_t groupType, char *buffer) {
	...
	if (groupType < 1 || groupType > nGroupTypes) {
		logError("Signal Error: Invalid group type");
		return false;
	}
...
bool getSignalTypeName(uint8_t signalType, char *buffer) {
	...
	if (signalType > nSignalTypes) {
		logError("Signal Error: Invalid signal type");
		return false;
	}
...
uint8_t getSizeOfDataTypeId(uint8_t dataTypeId) {
	switch (dataTypeId) {
		...
		default:
			logError("Signal Error: Data type id is %d unrecognized\n", dataTypeId);
...
bool mallocSignalSampleData(SignalSample *psig) {
	//logInfo("Signal: Allocating data for signal\n");
...
void freeSignalSampleData(SignalSample *psig) {
	//logInfo("Signal: Freeing data for signal\n");
	if (psig->data != NULL)
		FREE(psig->data);
	psig->dataBytes = 0;
}
...
timestamp_t pushTimestampToGroupInfo(GroupInfo *pg, timestamp_t ts, const SignalSample *signals, int nSignals) {
	...
	// first, check whether the group has any signals with type SIGNAL_TYPE_TIMESTAMPS
	...
	for (unsigned i = 0; i < nSignals; i++) {
		...
		if (ps->type == SIGNAL_TYPE_TIMESTAMP) {
			if (ps->dataTypeId != DTID_UINT32)
				logError("Signal Error: Signal with type TIMESTAMP must have data type uint32\n");
			else
				idxSignalTimestamp = i;
		}
		if (ps->type == SIGNAL_TYPE_TIMESTAMPOFFSET) {
			if (ps->dataTypeId != DTID_SINGLE)
				logError("SIgnal Error: Signal with type TIMESTAMPOFFSET must have data type single\n");
			else
...
bool checkSignalDataBufferMatchesSample(const SignalDataBuffer *psdb, const SignalSample *ps) {
	if (strncmp(ps->name, psdb->name, MAX_SIGNAL_NAME) != 0)
		return false;
	if (ps->dataTypeId != psdb->dataTypeId)
		return false;
	if (ps->nDims != psdb->nDims)
		return false;
	if (ps->pGroupInfo != psdb->pGroupInfo)
		return false;
...
bool pushSignalSampleToSignalDataBuffer(SignalDataBuffer *psdb, const SignalSample *ps) {
	...
	if ( !success ) {
		logError("Signal Error: Could not allocate memory for signal buffer %s\n", ps->name);
		return false;
	}
...
bool processControlSignalSamples(unsigned nSamples, const SignalSample *samples) {
	...
	for (unsigned i = 0; i < nSamples; i++) {
		...
		if (strncasecmp(ps->name, "protocol", MAX_SIGNAL_NAME) == 0) { // ignoring the case of caracters
			if (strncmp(dlStatus->protocol, (char*)ps->data, MAX_STRING) != 0) {
				...
				logInfo("Signal: Updating Protocol: %s\n", dlStatus->protocol);
			}
			...
			if (dlStatus->protocolVersion != ((uint32_t*)ps->data)[0]) {
				...
				logInfo("Signal: Updating Protocol Version: %d\n", dlStatus->protocolVersion);
			}
			...
			if (strncmp(dlStatus->subject, (char*)ps->data, MAX_STRING) != 0) {
				...
				logInfo("Signal: Updating Subject: %s\n", dlStatus->subject);
			}
			...
			if (strncmp(dlStatus->dataStore, (char*)ps->data, MAX_STRING) != 0) {
				...
				logInfo("Signal: Updating Data Store: %s\n", dlStatus->dataStore);
			}
			...
			if (dlStatus->saveTag != ((uint32_t*)ps->data)[0]) {
				...
				logInfo("Signal: Updating SaveTag: %d\n", dlStatus->saveTag);
			}
...
void controlInitialize(bool pendingNextTrial) {
	...
	if (dlStatusCurrent == NULL)
		diep("No memory to initialize data logger status");
...
// lock the mutex for this dlStatus, and then find the next non-utilized trial in the array of trials
...
int controlGetNextCompleteTrialToWrite(DataLoggerStatus *dlStatus) {
	...
	// this must find something since only one trial should be actively writing at any point in time
	bool found = false;
	unsigned newTrial;
	for (unsigned i = 0; i < BUFFER_NUM_TRIALS; i++) {
...
void controlManualSplitCurrentTrial() {
	//logInfo("Signal: Manually splitting current trial\n");
...
void controlFlushTrialsOlderThan(timestamp_t nSeconds) {
	...
	for (unsigned i = 0; i < BUFFER_NUM_TRIALS; i++) {
		if (!dlStatus->byTrial[i].activeWriting &&
			...
			logInfo("Signal: Flushing old trial data for buffered trial %d\n", i);
...
DataLoggerStatus *controlAdvanceToNewStatus() {
	...
	if (dlStatusNew == NULL)
		diep("No memory for new data logger status");
...
void controlPushRetiredStatus(DataLoggerStatus *dlStatus) {
	if (nStatusesRetired == STATUSES_RETIRED_NUM_BUFFER)
		diep("Ran out of room in retired status buffer");
...
unsigned controlAdvanceToNextTrial(uint32_t trialId, bool continuingSameTrial) {
	//logInfo("Signal: Advancing to new trial %d\n", trialId);
	DataLoggerStatus *dlStatus = controlGetCurrentStatus();
	...
	//logInfo("Signal: Advancing from trial %d to %d\n", lastTrial, newTrial);
	...
	if (dlStatus->byTrial[newTrial].utilized) {
		//logError("Signal Error: Overwriting unwritten trial with id=%d\n", dlStatus->byTrial[newTrial].trialId);
	}
...
/*
	if (autoTrialId)
		logInfo("Signal: Advancing to new trial, trialId = <automatic by time>\n");
	else
		logInfo("Signal: Advancing to new trial, trialId = %d\n", trialId);
*/
