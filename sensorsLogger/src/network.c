 ...
 * Modified  : Thu 25 Apr 2019 05:05:14 PM CEST
 * Computer  : ZVPIXX
...
static void (*packetSendCallbackFn)(const serialDevice*); // send sensor data to XPC
...
int startServer(const NetworkAddress *addr) {
	...
	hints.ai_flags = AI_PASSIVE;    // fill my IP address for me if host is NULL (wildcard IP address)
	...
	printf("Network: UDP server started at %s:%u\n", ipstr, addr->port);
...
void stopServer(const int sock) {
	printf("Network: Terminating UDP server\n");
...
void networkSetPacketSendCallbackFn(void (*fn)(const serialDevice*)) {
	packetSendCallbackFn = fn;
}

// automatically executed when a thread is canceled
static void networkThreadCleanup(void *arg) {
	printf("Network: ==> Terminating network thread\n");

	//serialList_t *s = (serialList_t*)arg;
	//int status = pthread_mutex_unlock(&s->mutex);
	//if (status != 0)
	//	err_abort(status, "Network: Unlock mutex in cleanup");
...
int networkThreadStart(const NetworkAddress* recv_addr, const NetworkAddress *send_addr, const serialList_t *s) {
	...
	int status = pthread_create(&(netThread.thread), NULL, networkThread, (void*)s);
	if (status) {
...
void networkThreadTerminate() {
	...
	if (res == PTHREAD_CANCELED)
		printf("Network: Network thread was canceled\n");
	else
		printf("Network: Network thread terminated normally\n");
}
...
static void *networkThread(void *arg) {
	...
	int bytesRecv = -1;
	struct timespec timeout; // condition timeout
	__attribute__((unused)) double ticCond = 0;
	while(1) {
		...
		timeout.tv_sec = time (NULL) + 2; // 2 sec
    timeout.tv_nsec = 0;

		// disable cancellation for a while, so that we can unlock mutex prior react to a cancellation request
		status = pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL);
		if (status != 0)
			err_abort(status, "Network: setcancelstate");

		status = pthread_mutex_lock(&s->mutex);
		if (status != 0)
			err_abort(status, "Network: Lock mutex");

		while (!s->flag) {
			status = pthread_cond_timedwait(&s->cond, &s->mutex, &timeout);
			if (status == ETIMEDOUT) {
#ifdef DEBUG
				if (ticCond < 0.0001)
					ticCond = getTime_msec();
				fprintf(stderr, \
						"\rNetwork: Sensor Mutex Condition wait timed out. (errorcode = %d) at \"%s\":%d : %.6f [sec]\n", \
						status, __FILE__, __LINE__, (getTime_msec()-ticCond)/1000);
        fflush(stderr);
#endif // DEBUG
				break;
			} else if (status != 0)
				err_abort(status, "Network: Wait on condition");
		}

		if (s->flag) {
			ticCond = 0;
			for (id = 0; id < s->num_tty; id++) { // signaled thread identification
				if (pthread_equal(s->tty[id]->thread, s->signaledThread)) {
					break;
				}
			}
			DPRINTF((" ==> FDC%u [%s] was signaled.\n", id, s->tty[id]->dev->name));

			// pass the sensor data to the callback function and send it to XPC
			if (packetSendCallbackFn != NULL) {
				if (s->tty[id] != NULL) {
					packetSendCallbackFn(s->tty[id]->dev);
				} else {
					fprintf(stderr, "Network: FDC%u is not found\n", id);
				}
			} else {
				fprintf(stderr, "Network: No packetSendCallbackFn specified!\n");
			}
		...
		if (status != 0)
			err_abort(status, "Network: Unlock mutex");

		// restore the cancelability of the thread
		status = pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);
		if (status != 0)
			err_abort(status, "Network: setcancelstate");
...
int networkRecv(PacketData *p, int bytesRecv, int flags) {
	...
	if (bytesRecv == -1) {
		DPRINTF(("Network: recvfrom() at \"%s\":%d : %s\n", __FILE__, __LINE__, strerror (errno)));
