 ...
 * Modified  : Fri 24 May 2019 05:48:47 PM CEST
 * Computer  : ZVPIXX
 ...
 *
 * LINUX note: Wireshark USB monitoring:
 *             add yourself to the wireshark group, logout/login: sudo usermod -a -G wireshark $USER
 *             dump USB traffic on Linux: sudo modprobe usbmon
 *             make the usbmonX device(s) readable: sudo setfacl -m u:$USER:r /dev/usbmon*
 */
...
double startDeviceStreaming(serialDevice *dev) {
	if ( !dev->isOpen ) {
		fprintf(stderr, "\tstartDeviceStreaming error: Open device %s first\n", dev->name);
		return -1;
	}
	...
	if ( !dev->isStreaming ) {
		...
		fprintf(stderr, "\t%s is already streaming\n", dev->name);
		return -1;
	}
...
double stopDeviceStreaming(serialDevice *dev) {
	...
	if ( dev->isStreaming ) {
		...
		fprintf(stderr, "\t%s is not streaming\n", dev->name);
		return -1;
	}
...
int getDeviceStreamingData(serialDevice *dev) {
	...
	for (int i = 0; i < 4; i++) {
		...
		if (dev->device_id == DEVICE_FDC2214) {
			...
			// update registers info
			strncpy(regBuf, &(dev->data.hexData[i])[2], 4);
			dev->registers[2*i] = (unsigned short int)strtol(regBuf, NULL, 16);
			dev->registers[2*i+1] = (unsigned short int)strtol(&(dev->data.hexData[i])[6], NULL, 16);
		} else if (dev->device_id == DEVICE_FDC2114) {
			strncpy(&(dev->data.hexData[i])[2], &hexRecv[12+i*8], 4);
			// update registers info
			dev->registers[2*i] = (unsigned short int)strtol(&(dev->data.hexData[i])[2], NULL, 16);
		}
...
int setRCount_ch(serialDevice *dev, unsigned int iChannel) {
	...
	switch (iChannel) {
		case 0:
			status = writeRegister(dev, RCOUNT_CH0, buf);
			break;
		case 1:
			status = writeRegister(dev, RCOUNT_CH1, buf);
			break;
		case 2:
			status = writeRegister(dev, RCOUNT_CH2, buf);
			break;
		case 3:
			status = writeRegister(dev, RCOUNT_CH3, buf);
			break;
	}
	return status;
}
...
int readAllRegisters(serialDevice *dev) {
	...
	strncpy(reg[33][0], "MANUFACTURER_ID", 15);    strncpy(reg[33][1], MANUFACTURER_ID, 2);
	strncpy(reg[34][0], "DEVICE_ID", 9);           strncpy(reg[34][1], DEVICE_ID, 2);

	printf(" Port = %s, Serial number = %s, Device ID = %d\n",
		 	dev->name, dev->serialNumber, dev->device_id);
	printf("           Register\tAddress\tCurrentValue\t  Bits\n");
	for (int i = 0; i < 35; i++) {
...
int writeRegister(serialDevice *dev, const char *addr, const char *data) {
	...
	// update serialDevice registers
	int idx = (int)strtol(addr, NULL, 16);
	if (idx == 126) // 0x7E - MANUFACTURER_ID
		idx = 33;
	if (idx == 127) // 0x7F - DEVICE_ID
		idx = 34;
	dev->registers[idx] = (unsigned short int)strtol(data, NULL, 16);
...
int readRegister(serialDevice *dev, const char *addr, char *data) {
	if ( !dev->isOpen ) {
		fprintf(stderr, "\treadRegister error: Open device %s first\n", dev->name);
		return -1;
	}
...
int queryData(const int fd, char *hexRecv, const char *hexSend, const bool addCRC8, const char *fromMsg) {
	...
	if (hexSend[strspn(hexSend, "0123456789abcdefABCDEF")] != 0) {
		fprintf(stderr,
				"\tError: queryData from %s. Input must be a valid hexadecimal character string: %lu (%lu).\n", fromMsg, strlen(hexSend), strspn(hexSend, "0123456789abcdefABCDEF"));
		return -1;
	}
	...
	int bSend;
	if ( fd_isValid(fd) ) {
		bSend = write(fd, asciiSend, str_len); /* use write() to send data to port */
	} else {
		fprintf(stderr,
				"\tError: queryData from %s. fileDescriptor is invalid.\n", fromMsg);
		return -1;
	}
	...
	*/
	...
	char asciiRecv[RECV_SIZE+1]; // it is always 32 char response
	int bRecv;
	if ( fd_isValid(fd) ) {
		bRecv  = read(fd, asciiRecv, RECV_SIZE);   /* returns after 32 chars have been input */
	} else {
		fprintf(stderr,
				"\tError: queryData from %s. fileDescriptor is invalid.\n", fromMsg);
		return -1;
...
int fd_isValid(int fd) {
	...
	if (flags == -1 )
		fprintf(stderr, "\n\tFcntl F_GETFL error (%d) for fd %d: %s\n", errno, fd, strerror(errno));
...
int open_device(serialDevice *dev, unsigned int baudRate) {
	if (dev->isOpen) {
		fprintf(stderr, "Warning: device %s is already opened\n", dev->name);
		return 0;
	}

	bool blocking = true;
	int fd = open(dev->name, O_RDWR | O_NOCTTY | (blocking ? 0 : O_NDELAY));
	...
	if (fd < 0) // error checking
		...
		printf("Sensors: %s opened successfully as %s\n", dev->name, ttyname(isatty(fd)));
	}
...
int close_device(serialDevice *dev) {
	...
	//tcsetattr(fd, TCSANOW, &(dev->saved_attributes));  // restore the terminal attributes (NOTE: block hotplug removing)
	
	int status = close(fd);
	if (status == -1)
		errno_print("Sensors: Close device error");
...
int set_interface_attribs(const int fd, unsigned int baudRate) {
	...
	if (tcgetattr(fd, &tty) < 0) {
		perror("Sensors: Failed to get the current configuration of the serial interface");
		return -1;
	}
	...
	if (cfsetispeed(&tty, (speed_t)baudRate) || cfsetospeed(&tty, (speed_t)baudRate)) {
		perror("Sensors: Invalid baud rate");
		return -1;
	} else {
		baudRate = cfgetispeed(&tty);
		//printf(" Set read/write speed to: %lu\n", (unsigned long)baudRate);
	}
	...
	if (tcsetattr(fd, TCSAFLUSH, &tty) < 0) {
		perror("Sensors: Error in setting attributes");
		return -1;
	} else
		printf(" BaudRate = %lu \n StopBits = 1 \n Parity   = none\n", (unsigned long)baudRate);
...
#if defined(__linux__)
  free(hwInfo->sysfs);
  free(hwInfo->sysfs_bus);
  free(hwInfo->sysfs_links);
#endif // __linux__
#if defined(__APPLE__)
	free(hwInfo->IOTTYBaseName);
#endif // __APPLE__
  free(hwInfo->manufact);
  free(hwInfo->product);
  free(hwInfo->serialNumber);
  free(hwInfo->driver);
}
