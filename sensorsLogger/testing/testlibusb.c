/*
 * File Name : testlibusb.c
 * Author    : Alexey Yu. Illarionov, INI UZH Zurich
 *             <ayuillarionov@ini.uzh.ch>
 *
 * Created   : Wed 20 Feb 2019 05:28:45 PM CET
 * Modified  : Wed 20 Feb 2019 07:28:57 PM CET
 * Computer  : ZVPIXX
 * System    : Linux 4.15.0-45-lowlatency x86_64 x86_64
 *
 * Purpose   : https://github.com/libusb/libusb/blob/master/examples/testlibusb.c
 */
	...
	if (ret == LIBUSB_SUCCESS) {
		...
		if (desc.iProduct) {
			...
			if (ret > 0)
				...
				snprintf(description + strlen(description), sizeof(description) - strlen(description),
						"%04X", desc.idProduct);
		}
		else
			snprintf(description + strlen(description), sizeof(description) - strlen(description),
