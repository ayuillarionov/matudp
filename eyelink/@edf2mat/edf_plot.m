function edf_plot(obj, startIdx, endIdx)
%EDF_PLOT plots the edf content to verify if they are useful
...
% #define INPUTEVENT   28  /* change of input port */
  ...
  assert(isa(obj, 'edf2mat'), 'edf2mat:edf_plot', ...
  ...
  assert(numel(range) > 0, ...
    'edf2mat:edf_plot:range','Start Index == End Index, nothing do be plotted');
  ...
  posX = obj.Samples.posXMM(range);
  posY = obj.Samples.posYMM(range);

  % Ploting Eye Movement
  subplot(2, 2, [1 3]);
  plot(posX, posY, 'o', 'Color','blue', 'MarkerSize', 3);

  title('Plot of the eye movement');
  %axis([min(posX)-1, max(posX)+1, min(posY)-1, max(posY)+1]);
  % fit to screen coordinate
  axis([-obj.screenWidthMM/2-1, obj.screenWidthMM/2+1, -obj.screenHeightMM/2-1, obj.screenHeightMM/2+1]);
  axis('square');
  xlabel('x [mm]');
  ylabel('y [mm]');
