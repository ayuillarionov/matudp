classdef CenterOutReachTask < DisplayTask
  ...
  methods
    ...
    function initialize(task, ~)
      ...
      task.center = Circle(0, 0, 0); % xc, yc, radius
      task.center.hide();
      task.dc.mgr.add(task.center);
      
      task.target = CircleTarget(0, 0, 0); % xc, yc, radius
      task.target.hide();
      ...
      task.hold = Rectangle(Inf,Inf,1,1); % xc, yc, width, height
      task.hold.hide();
      ...
      task.sound = AudioFeedback(); % loads wav files and plays sounds via PsychPortAudio
    end
