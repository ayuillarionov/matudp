...
% displays the screen half-rect and a 100 mm center square

  if nargin < 1
    screenIdx = max(Screen('Screens'));
  end

  % width and height of a window or screen in units of pixels
  [pw, ph] = Screen('WindowSize', screenIdx);
  % physical width and height of the output display device [mm]
  ...
  try
    ...
    Screen('DrawText', window, ...
      sprintf('screen size [%.0f, %.0f] pixels', pw, ph), 0, currentTextRow, white);
    currentTextRow = currentTextRow + 20;
    Screen('DrawText', window, ...
      sprintf('physical size [%.0f, %.0f] mm', mmWidth, mmHeight), 0, currentTextRow, white);
    currentTextRow = currentTextRow + 20;
    Screen('DrawText', window, ...
      'Press any key to exit.', 0, currentTextRow, white);
