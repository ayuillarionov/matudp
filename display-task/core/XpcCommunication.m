classdef XpcCommunication < handle
  ...
  methods
    ...
    function open(xcom)
      ...
      % Create a UDP socket and binds it to an UDP rxPort.
      % On this socket one can recive UDP packets destinated to this UDP port,
      % and send UDP packets with this sockat as source address.
      ...
      % 1. constructs an udp object associated with remote host and port
      xcom.udpSend = udp(xcom.targetIP, xcom.targetPort);
      % 2. bound it to the local socket
      fopen(xcom.udpSend);
    ...
    function close(xcom)
      try
        pnet(xcom.sock, 'close'); % close udpsocket
      catch
      end
    ...
    function str = readPacket(xcom)
      ...
      xcom.open();
      % Reads next incoming UDP packet on UDP socket "sock".
      ...
      if psize > 0
        ...
        if xcom.truncateNull % trims null (uint8(0)) from end of received packets
          lastInd = find(str > 0, 1, 'last');
          str = str(1:lastInd);
        end
      elseif pnet(xcom.sock, 'status') == 0 % peer has disconnect
    ...
    function [tags, groups] = readTaggedPacketsParseData(xcom)
      ...
      nTags = length(tags);
      removeFromTags = false(nTags, 1); % array of logical zeros
      for iTag = 1:nTags
        type = tags(iTag).type;
        str = tags(iTag).str;
        if strcmp(type, 'ds')
